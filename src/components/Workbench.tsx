...
export const Workbench: React.FC = () => {
  ...
  const getFirstFiveImages = React.useCallback((label: string) => {
    return aiImages.filter(img => Array.isArray(img.label) ? img.label.includes(label) : img.label === label).slice(0, 5);
  }, []);
  ...
  return <main className="min-h-screen flex items-center justify-center bg-hygge-1 px-2 py-[4px]">
      <div className="w-full max-w-5xl grid grid-cols-1 md:grid-cols-2 gap-10 items-start" style={{
      alignItems: "stretch"
    }}>
        ...
        <section className="flex flex-col gap-7">
          ...
          <div className="flex rounded-2xl shadow border font-semibold text-lg text-gray-900 bg-gradient-to-tr from-white via-white/90 to-white px-3 py-4 items-center w-full">
            ...
            <select className={`rounded-xl px-4 py-2 border-none font-bold text-base outline-none ml-2
                text-gray-900 shadow-sm cursor-pointer bg-gradient-to-tr transition-colors min-w-[150px]
                ${labelGradients[selected]} duration-200`} value={selected} onChange={e => setSelected(e.target.value)} aria-label="Select a label">
              {aiLabels.map(lbl => <option key={lbl.value} value={lbl.value} className="font-semibold" style={{
              color: "#291b1b",
          ...
          <div className="flex flex-col gap-3 w-full">
            {fourMessages.map((msg, idx) => msg && <div className="bg-white/85 rounded-lg px-4 py-2 text-base text-gray-800 font-playfair border border-gray-200 shadow" key={idx}>
                    {msg}
                  </div>)}
          </div>
        </section>
        ...
        <section className="flex flex-col gap-4">
          {/* Main image */}
          {mainImage && <div className="rounded-2xl overflow-hidden border border-white/60 shadow-xl mb-0 h-[240px] w-full flex justify-center items-center bg-white/60">
              <img src={mainImage} alt="Main generated" className="w-full h-full object-cover object-center" style={{
            maxHeight: 240,
          ...
          <div className="grid grid-cols-4 gap-3">
            {extraImages.map((img, i) => <div key={i} className="rounded-xl overflow-hidden border border-white/50 shadow bg-white/80 aspect-square flex items-center justify-center cursor-pointer hover:scale-105 transition-transform" style={{
            minHeight: 0
          }} onClick={() => handleExtraImageClick(i)} tabIndex={0} aria-label={`Use this image as main`} title="Click to make main image">
                <img src={img.url} alt={`Generated for ${selected}`} className="w-full h-full object-cover" draggable={false} style={{
              maxWidth: "100%",
              maxHeight: "100%"
            }} />
              </div>)}
